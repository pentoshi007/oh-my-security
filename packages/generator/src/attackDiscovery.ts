...
import { ATTACK_DATABASE, type AttackMethodology } from './attackDatabase.js';

interface DiscoveredAttack {
  name: string;
  description: string;
  category: string;
  confidence: number; // 0-1 score of how confident we are this is a new attack
  sources: string[]; // URLs where this attack was mentioned
  keywords: string[];
}

export class AttackDiscoveryService {
  private newsApiKey: string;
  private minConfidence = 0.7; // Minimum confidence to consider a new attack
   ...
   */
  async discoverNewAttacks(): Promise<DiscoveredAttack[]> {
   ...
   */
  private async extractAttackMethodologies(articles: any[]): Promise<DiscoveredAttack[]> {
    const discoveries: DiscoveredAttack[] = [];
   ...
   */
  private analyzeTextForAttacks(text: string, sourceUrl: string): DiscoveredAttack[] {
    const discoveries: DiscoveredAttack[] = [];
   ...
   */
  private calculateConfidence(attackName: string, text: string): number {
    ...
    const existingAttack = ATTACK_DATABASE.find(attack => 
      attack.name.toLowerCase().includes(lowerName) ||
    ...
    if (lowerText.includes('researchers') || lowerText.includes('security experts')) confidence += 0.2;
    if (lowerText.includes('discovered') || lowerText.includes('identified')) confidence += 0.15;
    if (lowerText.includes('novel') || lowerText.includes('new') || lowerText.includes('emerging')) confidence += 0.1;
    if (lowerText.includes('cve-') || lowerText.includes('vulnerability')) confidence += 0.1;
    if (lowerText.includes('malware') || lowerText.includes('ransomware')) confidence += 0.1;

    // Reduce confidence for generic terms
    if (['attack', 'threat', 'malware', 'virus', 'hack'].includes(lowerName)) confidence -= 0.3;
   ...
   */
  private formatAttackName(name: string): string {
    return name
      .replace(/[""']/g, '')
      .split(' ')
   ...
   */
  private extractDescription(attackName: string, text: string): string {
   ...
   */
  private categorizeAttack(attackName: string, text: string): string {
    const lowerName = attackName.toLowerCase();
    const lowerText = text.toLowerCase();

    if (lowerText.includes('ransomware') || lowerName.includes('ransom')) return 'Malware';
    if (lowerText.includes('phishing') || lowerName.includes('phish')) return 'Social Engineering';
    if (lowerText.includes('ddos') || lowerText.includes('denial of service')) return 'Network Attacks';
    if (lowerText.includes('sql') || lowerText.includes('xss') || lowerText.includes('injection')) return 'Web Application Attacks';
    if (lowerText.includes('apt') || lowerText.includes('advanced persistent')) return 'Advanced Attacks';
    if (lowerText.includes('iot') || lowerText.includes('smart device')) return 'IoT Attacks';
    if (lowerText.includes('crypto') || lowerText.includes('bitcoin') || lowerText.includes('blockchain')) return 'Cryptocurrency Attacks';
    if (lowerText.includes('social engineering') || lowerText.includes('human factor')) return 'Human Factor';
   ...
   */
  private extractKeywords(attackName: string, text: string): string[] {
   ...
   * Remove duplicates and filter by confidence
   */
  private deduplicateAndFilter(discoveries: DiscoveredAttack[]): DiscoveredAttack[] {
    const uniqueMap = new Map<string, DiscoveredAttack>();
