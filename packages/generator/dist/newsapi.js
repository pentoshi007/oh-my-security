...
export class NewsAPIService {
    ...
    scoreArticleRelevance(article, attack) {
        ...
        if (descLower.includes(attack.name.toLowerCase()))
            score += 6;
        if (contentLower.includes(attack.name.toLowerCase()))
            score += 4;
        ...
        attack.impacts.forEach(impact => {
            ...
            if (titleLower.includes(impactLower))
                score += 3;
            if (descLower.includes(impactLower))
                score += 2;
        });
        ...
        if (cybersecuritySources.some(source => article.source.name.toLowerCase().includes(source))) {
            score += 3;
        }
     ...
     */
    hasCybersecurityContext(text, keyword) {
