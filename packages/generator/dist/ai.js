...
export class AIContentGenerator {
    ...
    async generateBlueTeamContent(attack, newsArticles) {
...
Provide a detailed impact analysis, covering financial, operational, and strategic/reputational consequences. Include specific examples from the news articles about real-world impacts.`;
    ...
    async generateRedTeamContent(attack, newsArticles) {
        // Create a news context summary from the articles
        const newsContext = this.createNewsContext(newsArticles);
...
...
   ...
   Summary: ${article.description}`;
    ...
    parseBlueTeamContent(content, attackType) {
        const extractSection = (text, startMarker, endMarkers) => {
    ...
    parseRedTeamContent(content, attackType) {
        const extractSection = (text, startMarker, endMarkers) => {
            const startIndex = text.toLowerCase().indexOf(startMarker.toLowerCase());
            if (startIndex === -1)
                return '';
    ...
    getFallbackBlueTeamContent(attackType) {
        const fallbacks = {
            'Ransomware': {
    ...
    getFallbackRedTeamContent(attackType) {
        const fallbacks = {
            'Ransomware': {
